...
import com.victon.tpms.common.utils.SystemUtils;
...
public class NotificationReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        //判断app进程是否存活
        if(SystemUtils.isAppAlive(context, "com.xiaoan.tpms.usb")){
            //如果存活的话，就直接启动DetailActivity，但要考虑一种情况，就是app的进程虽然仍然在
            //但Task栈已经空了，比如用户点击Back键退出应用，但进程还没有被系统回收，如果直接启动
            //DetailActivity,再按Back键就不会返回MainActivity了。所以在启动
            //DetailActivity前，要先启动MainActivity。
            Log.i("NotificationReceiver", "the app process is alive");
            Intent mainIntent = new Intent(context, MainForServiceActivity.class);
...
//            detailIntent.putExtra("DB_ID", SharedPreferences.getInstance().getInt(Constants.LAST_ID,0));
...
//            context.startActivities(intents);
            ...
            Intent launchIntent = context.getPackageManager().
                    getLaunchIntentForPackage("com.xiaoan.tpms.usb");
