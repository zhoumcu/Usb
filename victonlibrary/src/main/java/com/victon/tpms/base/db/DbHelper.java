...
import com.victon.tpms.base.db.entity.CarBrand;
...
//                cursor.moveToNext();
    ...
    public List<CarBrand> getCarType(int id) {
        ...
        if(database==null) return new ArrayList<>();
        ArrayList<CarBrand> SourceDateList = null;
        Cursor cursor = null;
        try{
            cursor = database.rawQuery(sql, new String[]{String.valueOf(id)});
            String cname = "未找到该单词.";
...
//        database.execSQL(sql,new String[]{"good","good","good","good","good","good","good","good","good","good",});
            //getCarInfo();
            cursor = database.rawQuery("select max(id) as uid from car_info", null);
            cursor.moveToFirst();
            count = cursor.getInt(cursor.getColumnIndex("uid"));
        }finally {
            cursor.close();
            database.close();
            return count;
    ...
    public void insertCarData(RecordData data) throws SQLiteCantOpenDatabaseException {
        // 查找单词的SQL语句
        String sql = "insert into  car_data (deviceId,name,data,createDate,state,position) values (?,?,?,?,?,?)";
        ...
        try{
            ...
            cv.put("position", data.getViewPosition());
    ...
    public boolean update(int deviceId, String name, RecordData data) {
        ...
        try {
            ...
            cursor = database.rawQuery(sql, new String[]{String.valueOf(deviceId),name});
            // 必须使用moveToFirst方法将记录指针移动到第1条记录的位置
//        cursor.moveToFirst();
//            while (cursor.moveToNext()) {
...
//            }
    ...
    public List<RecordData> getCarDataList(int deviceId) throws SQLiteException {
        ...
        if(database==null) return new ArrayList<>();
        Cursor cursor = null;
        ArrayList<RecordData> carBrands = null;
        try {
            cursor = database.rawQuery(sql, new String[]{String.valueOf(deviceId)});
            String name = "未找到该单词.";
            String data = "未找到该单词.";
            carBrands = new ArrayList<>();
            // 必须使用moveToFirst方法将记录指针移动到第1条记录的位置
